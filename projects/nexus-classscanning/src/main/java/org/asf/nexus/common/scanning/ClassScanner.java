...
public class ClassScanner implements Closeable {
	...
	private FluidClassPool pool = FluidClassPool.createEmpty();
	...
	/**
	 * Adds the default classpath to the scanner
	 */
	public ClassScanner addDefaultCp() {
		if (imported)
		...
		addClassLoader(ClassLoader.getSystemClassLoader(), -10);
		return this;
	}
	 ...
	 */
	public ClassScanner addSource(File source) {
		if (imported)
			throw new IllegalArgumentException("Scanner already imported the class path");
	...
	/**
	 * Imports the entire classpath
	 */
	public ClassScanner importAll() {
		if (imported)
			return this;
	...
	private boolean verifyAnnotation(ClassNode node, String annotation) {
		// Check node
		annotation = annotation.replace("/", ".");
	...
	private boolean verifyTypeExtends(ClassNode node, String type) {
		// Check node
		type = type.replace("/", ".");
